...
class ModuleCategoryRepository 
{
  async ModuleCategorydata(){
    try{
      return await sequelize.query('EXEC sp_FatchModuleCategoryData');
    }
    catch(error){
      console.log(error);
      throw(error);
  ...
  async InsertUpdateModuleCategory(jsonData) {
    try {
      const query = `EXEC InsertUpdateModuleCategory @jsonData = :jsonData`;
