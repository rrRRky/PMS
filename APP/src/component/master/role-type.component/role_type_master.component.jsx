...
import AddRoleComponent from './add_role_type.component';
...
class BtnCellRenderer extends Component {
  ...
  btnClickedHandler = _.debounce((rolesId, action) => {
    ...
    const buttonMap = {
      'E': (
        <Button
          ...
          onClick={() => this.btnClickedHandler(rolesId, 'edit')}
        >
          <Link to={`/edit-role/${rolesId}`}>
            <FontAwesomeIcon
    ...
    return (
      <span className='d-flex justify-content-between align-items-center' style={{height:"44px"}} onClick={() => this.btnClickedHandler(roleId, 'edit')}>
         ...
         style={{ minWidth: '40px', height: '44px' }}
          onClick={() => this.btnClickedHandler(rolesId, 'edit')}
         >
           <Link  to={`/roles-and-permission/${rolesId}`}>
            <FontAwesomeIcon
