...
const AllSubSpaceStageComponent = () => {
    ...
    useEffect(() => {
      ...
      const fetchData = async (subspacestageId) => {
        try { 
            const response = await axios.get(`${API_URL}SpcLstStg?spaceListId=0&spaceListStageId=0`, {
                headers: {
...
class BtnCellRenderer extends Component {
  ...
  render() {
    ...
    console.log(data);
    const subspacestageId = data.id;
    const ModuleRights = this.state.ModuleRights; 
    const spaceListID = data.spaceListId;
    const buttonMap = {
      'E': (
        <Button
          key="editButton"
          ...
          onClick={() => { console.log('Edit button clicked'); this.btnClickedHandler(spaceListID/subspacestageId, 'edit');}}
        >
          <Link  to={`/edit-sub-space-stage/${spaceListID}/${subspacestageId}`}>
            <FontAwesomeIcon
    ...
    return (
      <span className='d-flex justify-content-between align-items-center' style={{height:"44px"}} onClick={() => this.btnClickedHandler(spaceListID, 'edit')}>
...
export default AllSubSpaceStageComponent;
