...
import AddSubSpaceComponent from './add-sub-space.component';
...
const SubSpaceComponent = () => {
    ...
    useEffect(() => {
      ...
      const fetchData = async () => {
        try { 
            const response = await axios.get(API_URL + 'SpaceLists', {
                headers: {
    ...
    return (
      <span className='d-flex justify-content-between align-items-center' style={{height:"44px"}} onClick={() => this.btnClickedTempHandler(roleId, 'edit')}>
        <Button
          ...
          variant='outline'
          style={{ minWidth: '40px', height: '44px' }}
          onClick={() => this.btnClickedTempHandler(subSpaceId, 'view')}
        >
          <Link to={`/sub-space-stage/${subSpaceId}`}>
            <FontAwesomeIcon
              className='text-primary'
...
export default SubSpaceComponent;
